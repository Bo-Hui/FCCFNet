...
class DBFF(nn.Module):
    ...
    def __init__(self, in_channel, kernel_size=1, stride=1):
        ...
        self.encoder_ca = MultiDimCA(self.inter_channel,reduction=4)
        self.decoder_ca = MultiDimCA(self.inter_channel,reduction=4)
...
class Net(nn.Module):
    ...
    def __init__(self, cfg: dict, out_ch: int = 1):
        ...
        for c in cfg["encode"]:
            ...
            se_list.append(CGAFusion(last_c,c[3],reduction=8))#输入输出通道大小一致，所以对齐
            last_c = c[3]
